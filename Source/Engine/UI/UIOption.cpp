...
void UIOption::Render(ID3D11DeviceContext* dc, float screenW, float screenH, float alpha) const noexcept
{
    ...
    // Use the shared base dimensions 
    const float scaleX{ screenW / UI::s_baseWidth };
    const float scaleY{ screenH / UI::s_baseHeight };

    // Leverage the helper inside the lambda to keep all primitive scaling uniform
    auto DrawScaledRect = [&](float x, float y, float w, float h, float r, float g, float b, float a) {
        const auto scaled = UI::GetScaled(x, y, w, h, screenW, screenH);
        m_primitive->Rect(scaled.x, scaled.y, scaled.w, scaled.h, 0.0f, 0.0f, 0.0f, r, g, b, a);
        };

    // Background, Frame, & Header Separator 
    ...
    DrawScaledRect(innerX, innerY, innerW, innerH, BG_R, BG_G, BG_B, alpha * BG_ALPHA_MULTIPLIER);
    DrawScaledRect(PANEL_POS_X, PANEL_POS_Y, PANEL_WIDTH, BORDER_THICKNESS, OUTLINE_R, OUTLINE_G, OUTLINE_B, alpha);
    DrawScaledRect(PANEL_POS_X, PANEL_POS_Y + PANEL_HEIGHT - BORDER_THICKNESS, PANEL_WIDTH, BORDER_THICKNESS, OUTLINE_R, OUTLINE_G, OUTLINE_B, alpha);
    DrawScaledRect(PANEL_POS_X, PANEL_POS_Y + BORDER_THICKNESS, BORDER_THICKNESS, PANEL_HEIGHT - (BORDER_THICKNESS * 2.0f), OUTLINE_R, OUTLINE_G, OUTLINE_B, alpha);
    DrawScaledRect(PANEL_POS_X + PANEL_WIDTH - BORDER_THICKNESS, PANEL_POS_Y + BORDER_THICKNESS, BORDER_THICKNESS, PANEL_HEIGHT - (BORDER_THICKNESS * 2.0f), OUTLINE_R, OUTLINE_G, OUTLINE_B, alpha);
    DrawScaledRect(innerX, PANEL_POS_Y + SEPARATOR_OFFSET_Y, innerW, SEPARATOR_THICKNESS, OUTLINE_R, OUTLINE_G, OUTLINE_B, alpha);
    ...
    for (std::size_t i = 0; i < static_cast<std::size_t>(OptionRow::Count); ++i)
    {
        ...
        DrawScaledRect(trackPosX, trackPosY, SLIDER_WIDTH, SLIDER_HEIGHT, 0.3f, 0.3f, 0.3f, alpha);
        ...
        DrawScaledRect(knobPosX, knobPosY, KNOB_WIDTH, KNOB_HEIGHT, r, g, b, alpha);
    }
    ...
    if (m_font)
    {
        // Use the smaller dimension multiplier to ensure font text doesn't stretch or warp strangely
        const float fontScale = (std::min)(scaleX, scaleY);

        m_font->Draw(m_titleText, (PANEL_POS_X + TITLE_OFFSET_X) * scaleX, (PANEL_POS_Y + TITLE_OFFSET_Y) * scaleY, fontScale, { 1.0f, 1.0f, 1.0f, alpha });
        ...
        for (std::size_t i = 0; i < static_cast<std::size_t>(OptionRow::Count); ++i)
        {
            ...
            m_font->Draw(m_rowTexts[i], labelPosX * scaleX, labelPosY * scaleY, fontScale, textColor);
            m_font->Draw(m_volumeStrings[m_volumes[i]], valuePosX * scaleX, labelPosY * scaleY, fontScale, textColor);
        }
    }
}